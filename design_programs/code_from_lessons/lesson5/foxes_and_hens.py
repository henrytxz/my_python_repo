...
def do(action, state):
    ...
    def remove(cards, card):
        if card is 'F':
            ...
            raise RuntimeError('unexpected card {0} found'.format(card))
    ...
    if action == 'wait':
        ...
        if card is 'F':
            ...
            raise RuntimeError('unexpected card {0}'.format(card))
    elif action == 'gather':
        return score+yard, 0, cards
    else:
        raise RuntimeError('{0} is not one of the allowed actions'.format(action))
...
def superior(A, B=take5):
    ...
    print 'A {0}'.format(a)
    print 'B {0}'.format(b)
    print 'A {0} better than take5'.format(a-b)
    return a - b > 1.5
