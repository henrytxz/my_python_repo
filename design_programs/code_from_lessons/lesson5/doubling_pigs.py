...
def Q_pig_d(state, action, U):
    if action == 'hold':
        return 1 - Pwin_d(do(action, state, dierolls())) # when action is hold, I know dierolls won't be used so this is not an expectation, it is deterministic
    elif action == 'roll':
        return (1-Pwin_d() + sum(Pwin_d(do(action, state, iter([i,]))) for i in range(2,7)))/6
    # pass
...
def Pwin_d(state):
    ...
    (p, me, you, pending, double) = state
    print 'p {0} me {1} you {2} pending {3} double {4}'.format(p, me, you, pending, double)
    if me + pending >= goal:
        return 1
    elif you >= goal:
        return 0
...
def test_Pwin_d():
    state = (0, 36, 35, 0, 1)
    # assert Pwin_d(do('roll', state, iter([1,]))) < 1
...
def test():
    ...
    # print strategy_compare(strategy_d, hold_20_d) # must win 60% of the points
    best_record = 0.0
    for n in range(0, 40, 1):
        x = strategy_compare(hold_at(n), hold_20_d)
        print 'hold at {0} wins {1}'.format(n, x)
        if x > best_record:
            best_record = x
