...
def lowest_cost_search(start, successors, is_goal, action_cost):
    ...
    while frontier:
        ...
        for state, action in successors(state1).iteritems():
...
def all_over(state):
    print 'in all_over, state is {0}'.format(state)
    here, there = state
    return len(here) == 0

lowest_cost_search((frozenset([1,2,5,10,'light']), frozenset()),
                   bsuccessors2, all_over, bcost)