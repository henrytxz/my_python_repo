...
def bsuccessors3(state):
    ...
    if light == 0 :
        arrow = '->'
        return {(here-frozenset([a,b]), there|frozenset([a,b]), 1) : (set([a,b]), arrow) for a in here for b in here}
    else:
        arrow = '<-'
        return {(here|frozenset([a,b]), there-frozenset([a,b]), 0) : (set([a,b]), arrow) for a in there for b in there}
