...
class Products(object):
    ...
    def set_lp(self, x, product_i_minus_1=None):
    ...
    def set_sp(self, x, product_i_minus_1=None):
        if product_i_minus_1 is None:
...
def largest_product_of_k_ints(A, k):
    ...
    for i in range(k):
        first_i_prods *= A[i]
        products[i].set_sp(first_i_prods)
        products[i].set_lp(first_i_prods)
    for i in range(len(A)):
        for j in range(k-1, -1, -1):
            ...
            if i==0:
                ...
                last_product = products[j-1]
            products[j].set_sp(A[i], last_product)
            products[j].set_lp(A[i], last_product)
...
if __name__ == '__main__':
    print highest_product([-3, -2, 0, 4, 5])